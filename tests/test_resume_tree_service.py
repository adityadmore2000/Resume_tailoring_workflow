...
async def test_insert_section_experience_bullet(db_session):
    ...
    item = await svc.insert_node(section.id, NodeCreate(node_type="item", title="Acme"))
    detail = await svc.insert_node(item.id, NodeCreate(node_type="detail", content={"text": "Did X"}, metadata={"searchable": True, "source_text": "Did X"}))

    rows = (
        await db_session.execute(select(ResumeNode).where(ResumeNode.resume_id == resume_id))
    ).scalars().all()
    assert {n.id for n in rows} == {root_id, section.id, item.id, detail.id}
...
async def test_update_leaf_only(db_session):
    ...
    detail = await svc.insert_node(section.id, NodeCreate(node_type="detail", content={"text": "old"}, metadata={"searchable": True, "source_text": "old"}))

    await svc.update_node(detail.id, NodePatch(content={"text": "new"}, metadata={"searchable": True, "source_text": "new", "a": 1}))

    refreshed_section = await db_session.get(ResumeNode, section.id)
    refreshed_bullet = await db_session.get(ResumeNode, detail.id)
...
async def test_delete_leaf(db_session):
    ...
    section = await svc.insert_node(root_id, NodeCreate(node_type="section", title="S"))
    bullet = await svc.insert_node(section.id, NodeCreate(node_type="detail", content={"text": "x"}, metadata={"searchable": True, "source_text": "x"}))
...
async def test_delete_subtree(db_session):
    ...
    section = await svc.insert_node(root_id, NodeCreate(node_type="section", title="S"))
    item = await svc.insert_node(section.id, NodeCreate(node_type="item", title="E"))
    bullet = await svc.insert_node(item.id, NodeCreate(node_type="detail", content={"t": "x"}, metadata={"searchable": True, "source_text": "x"}))

    await svc.delete_subtree(section.id)
    assert await db_session.get(ResumeNode, section.id) is None
    assert await db_session.get(ResumeNode, item.id) is None
    assert await db_session.get(ResumeNode, bullet.id) is None
...
async def test_move_node(db_session):
    ...
    exp = await svc.insert_node(s1.id, NodeCreate(node_type="item", title="E"))
...
async def test_retrieve_full_resume_tree(db_session):
    ...
    section = await svc.insert_node(root_id, NodeCreate(node_type="section", title="S"))
    _ = await svc.insert_node(section.id, NodeCreate(node_type="detail", content={"text": "x"}, metadata={"searchable": True, "source_text": "x"}))
